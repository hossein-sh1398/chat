    ...
    <!--begin::Card header-->
    <div class="card-header pt-10px" id="kt_chat_messenger_header">
        <!--begin::Title-->
        <div class="card-title">
            ...
            <div class="d-flex justify-content-center flex-column me-3">
                ...
                <!--begin::Info-->
                <div class="mb-1 lh-1 {{ $user->chatStatus && $user->chatStatus->is_online ? '' : 'd-none' }}" id="message-list-online-user-{{ $user->id }}">
                    <span class="badge badge-success badge-circle w-10px h-10px me-1"></span>
                    <span class="fs-7 fw-semibold text-muted">فعال</span>
                </div>
                <div style="height: 20px;">
                    <span id="is-typing-icon-{{ $user->id }}" class="badge badge-success is-typing-icon {{ $user->chatStatus && $user->chatStatus->is_typing ? '' : 'd-none' }}">مشغول تایپ ...</span>
                </div>
                <!--end::Info-->
            </div>
            <!--end::User-->
        ...
        <div class="card-toolbar">
            <!--begin::Menu-->
            <div class="me-n3">
                <div class="d-flex justify-content-center align-items-center gap-3 video-audio-call-div">
                    @if (! $user->isCalling() && ! auth()->user()->isCalling())
                        <span id="btn-video-call" onclick="startVideoCall(event)" class="btn btn-success btn-sm ml-5 btn-hover-rise p-2 fs-8 mb-3 btn-call"><i class="fa fa-video"></i></span>
                        <span id="btn-phone-call" onclick="startVoiceCall(event)" class="btn btn-primary btn-sm ml-5 btn-hover-rise p-2 fs-8 mb-3 btn-call"><i class="fa fa-phone"></i></span>
                    @elseif ($user->isCalling())
                        <span class="badge badge-info is-calling-icon">در حال مکالمه</span>
                    @endif
                </div>
                <!--end::Menu 3-->
            </div>
    ...
    <div class="card-body" id="kt_chat_messenger_body">
        <button onclick="readNewMessage()" class="btn btn-sm btn-primary btn-new-message-incoming d-none">
            <div>
                <span></span>
        ...
        <!--begin::Messages-->
        <div class="me-n5 pe-5" onscroll="scrollChatMessageBox(event, '{{ route('admin.chat.messages', ['user' => $user->id]) }}', {{ $user->id }})" id="chat-messages-list" data-kt-element="messages" data-kt-scroll="true" data-kt-scroll-activate="{default: false, lg: true}" data-kt-scroll-max-height="auto" data-kt-scroll-dependencies="#kt_header, #kt_toolbar, #kt_footer, #kt_chat_messenger_header, #kt_chat_messenger_footer" data-kt-scroll-wrappers="#kt_content, #kt_chat_messenger_body" data-kt-scroll-offset="5px">
            @include('admin.pages.chat.message', ['lists' => $lists, 'user' => $user])
        </div>
        <!--end::Messages-->
    </div>
    <!--end::Card body-->
    ...
    <div class="card-footer pt-4">
        <div class="mb-6 pb-8 bb-1px-gray">
            <input type="hidden" id="active-user-sidebar" value="{{ $user->id }}">
            <form id="form-send-chat-message" action="{{ route('admin.chat.send.message', $user->id) }}" method="post" enctype="multipart/form-data">
                @csrf
                <!--begin::Input-->
                <input type="text" autocomplete="off" class="form-control form-control-flush mb-3" name="message" onkeydown="isTypeing()" id="chat-message-input" placeholder="چیزی بنویسد..." />
                <!--end::Input-->
                <!--begin:Toolbar-->
                <div class="d-flex flex-stack">
                    <!--begin::Actions-->
                    <div class="d-flex align-items-center me-2">
                        <button class="btn btn-sm btn-icon btn-active-light-primary me-1" onclick="$('#file-input').click()" type="button" data-bs-toggle="tooltip">
                        ...
                        <div class="audio-recording-container">
                            ...
                            <div class="recording-contorl-buttons-container hide">
                                <i class="cancel-recording-button fa fa-remove" aria-hidden="true"></i>
                        ...
                        <input type="file" name="file" id="file-input" class="d-none" onchange="loadFile(event , 'chat')">
                    </div>
                    <!--end::Actions-->
                    <!--begin::Send-->
                    <button class="btn btn-success btn-sm ml-5 btn-hover-rise p-2 fs-8 mb-3 form-btn-send-message" type="submit"><i class="fas fa-paper-plane"></i></button>
                    <div id="spinner-border-text-danger" class="spinner-border text-danger d-none" role="status"></div>
                    <!--end::Send-->
                </div>
                <!--end::Toolbar-->
            </form>
    ...
    <!--end::Card footer-->
<script>
    localStorage.setItem('active-user-sidebar', {{ $user->id }});
    var audioRecorder = {
    ...
    audioRecorder
        ...
        switch (error.name) {
            ...
            toastr.error('هیچ میکروفونی پیدا نشد.');
    ...
    reader.onload = (e) => {
        ...
        audioElement.load();
        var fd = new FormData();
        fd.append('file', recorderAudioAsBlob);
        fd.append('message', '');
        fd.append('name', 'hossein.ogg');
        fd.append('type', BlobType);
        let __url = "{{ route('admin.chat.send.message', 'user_0') }}";
        __url = __url.replace('user_0', $('#active-user-sidebar').val())
        $.ajax({
            url:__url,
            type:'POST',
            contentType: false,
